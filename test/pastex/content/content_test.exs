defmodule Pastex.ContentTest do
  ...
  describe "pastes" do
    ...
    test "get_paste!/1 returns the paste with given id" do
      paste = paste_fixture()
      assert Content.get_paste(paste.id) == {:ok, paste}
    end
    ...
    test "update_paste/2 with invalid data returns error changeset" do
      ...
      assert {:ok, paste} == Content.get_paste(paste.id)
    end

    test "delete_paste/1 deletes the paste" do
      paste = paste_fixture()
      assert {:ok, %Paste{}} = Content.delete_paste(paste)
      assert {:error, _reason} = Content.get_paste(paste.id) end
    end
  ...
  describe "files" do
    ...
    test "get_file!/1 returns the file with given id" do
      file = file_fixture()
      assert Content.get_file(file.id) == {:ok, file}
    end
    ...
    test "update_file/2 with invalid data returns error changeset" do
      ...
      assert {:ok, file} == Content.get_file(file.id)
    end

    test "delete_file/1 deletes the file" do
      file = file_fixture()
      assert {:ok, %File{}} = Content.delete_file(file)
      assert {:error, _reason} == Content.get_file(file.id) end
    end
